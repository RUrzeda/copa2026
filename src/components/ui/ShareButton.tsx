...
export function ShareMatch({ match }: ShareMatchProps) {
  ...
  const url = 'https://paineldacopa.com.br'
  ...
  return (
    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 sm:opacity-100 transition-opacity">
      <a
        ...
        className="p-1.5 rounded-lg text-slate-600 hover:text-green-400 hover:bg-green-400/10 transition-all"
      ...
      <a
        ...
        className="p-1.5 rounded-lg text-slate-600 hover:text-slate-300 hover:bg-slate-400/10 transition-all"
        onClick={e => e.stopPropagation()}
      >
        {/* X (Twitter) icon */}
        <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="currentColor">
...
// Botão genérico para compartilhar o site
export function ShareSite() {
  const text = 'Acompanhe a Copa do Mundo 2026 em tempo real — resultados, grupos e artilheiros atualizados automaticamente!'
  const url  = 'https://paineldacopa.com.br'
